 ...
 * @ignore
 */
function yam_set_var($params, &$smarty)
{
  $map = '';
...
/*---------------------------------------------------------
 cggm2_add_dynpoint()
 ---------------------------------------------------------*/
function yam_add_dynpoint($params, &$smarty)
{
  ...
  if( isset($params['map']) ) {
    if( !is_numeric($params['map']) ) {
      $mapobj = yam_map_operations::load_by_name($params['map']);
  ...
  try {
      if( !$address && !$lat && !$long ) throw new \Exception('Error: invalid parameters passed to yam_add_dynpoint');
      $obj = new yam_marker($name,$address,$lat,$long,$icon);
...
function yam_add_kml($params, &$smarty)
{
  $mod = cge_utils::get_module('YandexMaps');
  if( !$mod ) return;
  $map = '';
  ...
  if( !is_numeric($map) ) {
    $mapobj = yam_map_operations::load_by_name($map);
    $map = $mapobj->id;
  }
...
/* ---- Begin Class --- */
final class YandexMaps extends CGExtensions
{
  var $_dynamic_points;
  var $_dyn_categories;
  var $_kml_files;
  ...
  function __construct()
  {
    ...
    $smarty->register_function('yam_set_var','yam_set_var');
    $smarty->register_function('yam_add_dynpoint','yam_add_dynpoint');
    $smarty->register_function('yam_add_kml','yam_add_kml');
  }

  function GetName() { return 'YandexMaps'; }
  function GetFriendlyName() { return $this->Lang('friendlyname'); }
  function GetVersion() { return '0.1'; }
  function GetHelp() { return file_get_contents(__DIR__.'/help.inc'); }
  function GetAuthor() { return 'sergvic'; }
  ...
  function GetDependencies() { return array('CGExtensions'=>'1.40','CGSimpleSmarty'=>'1.7.3'); }
...
EOT;
  ...
  {
    $tmp = yam_address_lookup::lookup($address);
    if( is_array($tmp) ) {
      $data = array();
      $data['lat'] = $tmp[0];
      $data['lon'] = $tmp[1];
  ...
  public function add_map_point($map,yam_marker& $obj)
  {
    if( !is_array($this->_dynamic_points) ) $this->_dynamic_points = array();
    if( !isset($this->_dynamic_points[$map]) ) $this->_dynamic_points[$map] = array();
  ...
  public function add_map_category($map,yam_category& $obj)
  {
    if( $map == '' ) return FALSE;
