...
final class yam_map_operations
{
  ...
  public static function update_from_formdata(yam_map& $map,$formdata)
  {
  ...
  private static function map_to_row(yam_map& $map)
  {
    $data = array();
    $data[] = $map->get_name();
  ...
  public static function &load_by_id($map_id,$deep = false)
  {
    ...
    if( !isset(self::$_maps[$map_id]) ) {
      $db = cmsms()->GetDb();
      $query = 'SELECT * FROM '.cms_db_prefix().'module_yandexmaps WHERE map_id = ?';
    ...
    if( $deep && !self::$_maps[$map_id]->count_markers() ) yam_marker_operations::load_markers_for_map(self::$_maps[$map_id]);
  ...
  public static function &load_by_name($map_name,$deep = false)
  {
    ...
    if( !is_array(self::$_map_name_map) ) {
      $query = 'SELECT map_id,name FROM '.cms_db_prefix().'module_yandexmaps';
  ...
  public static function delete(yam_map& $map)
  {
    $gCms = cmsms();
    $db = $gCms->GetDb();
    $map_id = $map->get_id();

    $query = 'DELETE FROM '.cms_db_prefix().'module_yandexmaps_points WHERE map_id = ?';
    $db->Execute($query,array($map_id));

    $query = 'DELETE FROM '.cms_db_prefix().'module_yandexmaps WHERE map_id = ?';
    $db->Execute($query,array($map_id));

    $mod = cms_utils::get_module('YandexMaps');
  ...
  public static function insert(yam_map& $map,$deep = true)
  {
    $query = 'INSERT '.cms_db_prefix().'module_yandexmaps (name,owner_id,data) VALUES (?,?,?)';
	...
	yam_marker_operations::insert($marker);
      }
    }
    ...
    $query = 'SELECT COUNT(map_id) FROM '.cms_db_prefix().'module_yandexmaps';
    $tmp = $db->GetOne($query);
    $mod = cms_utils::get_module('YandexMaps');
  ...
  public static function update(yam_map& $map,$deep = true)
  {
    $query = 'UPDATE '.cms_db_prefix().'module_yandexmaps SET name = ?, owner_id = ?, data = ? WHERE map_id = ?';
    $gCms = cmsms();
    $db = $gCms->GetDb();
	...
	$marker = $map->get_marker_by_idx($i);
	$marker->set_map_id($map_id);
	yam_marker_operations::update($marker);
      }
    }
